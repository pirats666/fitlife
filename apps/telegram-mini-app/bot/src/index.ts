...
async function sendStart(chatId: number, firstName?: string) {
  await telegram('sendMessage', { chat_id: chatId, text: `Привет${firstName ? `, ${firstName}` : ''}! 👋\n\nДобро пожаловать в FitLife. Здесь твои тренировки, расписание и прогресс в одном месте.`, reply_markup: { inline_keyboard: [[{ text: '🚀 Открыть FitLife', web_app: { url: webAppUrl } }]] } });
}

async function sendHelp(chatId: number) {
  await telegram('sendMessage', { chat_id: chatId, text: 'FitLife помогает запускать тренировки, видеть расписание и отслеживать прогресс.\n\n/start — открыть FitLife\n/help — помощь\n/profile — открыть приложение', reply_markup: { inline_keyboard: [[{ text: 'Открыть приложение', web_app: { url: webAppUrl } }]] } });
}
  ...
  while (true) {
    try {
      ...
      for (const update of updates) {
        ...
        if (command === '/start' || command === '/profile') await sendStart(message.chat.id, message.from?.first_name);
