...
const screens = {
  ...
  schedule: { title: () => 'Расписание', subtitle: 'План тренировок', html: () => `<section class="list-card"><div><b>Сегодня</b><span>Тренировка доступна</span></div><span class="status">Готово</span></section>` },
  progress: { title: () => 'Прогресс', subtitle: 'История твоих тренировок', html: () => `<section class="stats"><div><b id="workoutCount">—</b><span>тренировок</span></div><div><b>—</b><span>недель</span></div><div><b>✓</b><span>последняя</span></div></section><section class="list-card"><div><b>История</b><span id="historyText">Загрузка…</span></div></section>` },
...
function renderExercise() {
  ...
  if (e.mode === 'seconds') {
    timer = setInterval(() => { remaining--; const node = document.getElementById('exerciseTimer'); if (node) node.textContent = `${remaining}с`; if (remaining <= 0) clearInterval(timer); }, 1000);
  }
...
async function loadHistory() {
  ...
  try { const history = (await api(`/api/users/${fitLifeUser.id}/workouts/history`)).history; document.getElementById('workoutCount').textContent = history.length; document.getElementById('historyText').textContent = history.length ? `Последняя тренировка: ${new Date(history[0].completedAt).toLocaleString('ru-RU')}` : 'Пока нет завершённых тренировок'; }
...
async function boot() {
  try { fitLifeUser = await authenticate(); await loadWorkouts(); if (!fitLifeUser?.onboardingCompleted) render('onboarding'); else render('home'); }
