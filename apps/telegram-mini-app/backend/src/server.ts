...
function trainerAuthorized(request: { headers: Record<string, string | string[] | undefined> }, trainerId: number) {
  const authUser = authenticatedUser(headerInitData(request));
  return authUser?.id === trainerId && isTrainer(trainerId);
}
...
app.post<{ Body: { initData?: string } }>('/api/auth/telegram', async (request, reply) => {
  try { return { ok: true, user: upsertUser(validateTelegramInitData(request.body?.initData ?? '', process.env.TELEGRAM_BOT_TOKEN ?? '')) }; }
...
app.get<{ Params: { telegramId: string } }>('/api/users/:telegramId', async (request, reply) => {
  const telegramId = Number(request.params.telegramId); const authUser = authenticatedUser(headerInitData(request));
  if (!Number.isSafeInteger(telegramId) || !authUser || authUser.id !== telegramId) return reply.code(403).send({ ok: false, error: 'Forbidden' });
  const user = getUser(telegramId); if (!user) return reply.code(404).send({ ok: false, error: 'User not found' }); return { ok: true, user };
});
app.patch<{ Params: { telegramId: string }; Body: { goal?: 'health' | 'strength' | 'fitness'; onboardingCompleted?: boolean } }>('/api/users/:telegramId', async (request, reply) => {
  const telegramId = Number(request.params.telegramId); const authUser = authenticatedUser(headerInitData(request));
  if (!Number.isSafeInteger(telegramId) || !authUser || authUser.id !== telegramId) return reply.code(403).send({ ok: false, error: 'Forbidden' });
  const user = updateUser(telegramId, request.body ?? {}); if (!user) return reply.code(404).send({ ok: false, error: 'User not found' }); return { ok: true, user };
});
app.post<{ Params: { telegramId: string; workoutId: string } }>('/api/users/:telegramId/workouts/:workoutId/complete', async (request, reply) => {
  ...
  const history = completeWorkout(telegramId, request.params.workoutId); if (!history) return reply.code(404).send({ ok: false, error: 'User not found' }); return { ok: true, history };
});
app.get<{ Params: { telegramId: string } }>('/api/users/:telegramId/workouts/history', async (request, reply) => {
  const telegramId = Number(request.params.telegramId); const authUser = authenticatedUser(headerInitData(request));
  if (!Number.isSafeInteger(telegramId) || !authUser || authUser.id !== telegramId) return reply.code(403).send({ ok: false, error: 'Forbidden' });
  const history = getWorkoutHistory(telegramId); if (!history) return reply.code(404).send({ ok: false, error: 'User not found' }); return { ok: true, history };
});
...
app.get<{ Params: { telegramId: string } }>('/api/users/:telegramId/schedule', async (request, reply) => {
  const telegramId = Number(request.params.telegramId); const authUser = authenticatedUser(headerInitData(request));
  if (!Number.isSafeInteger(telegramId) || !authUser || authUser.id !== telegramId) return reply.code(403).send({ ok: false, error: 'Forbidden' });
  return { ok: true, schedule: getClientSchedule(telegramId) };
});
app.get<{ Params: { telegramId: string } }>('/api/users/:telegramId/progress', async (request, reply) => {
  const telegramId = Number(request.params.telegramId); const authUser = authenticatedUser(headerInitData(request));
  if (!Number.isSafeInteger(telegramId) || !authUser || authUser.id !== telegramId) return reply.code(403).send({ ok: false, error: 'Forbidden' });
  const history = getWorkoutHistory(telegramId); if (!history) return reply.code(404).send({ ok: false, error: 'User not found' });
...
app.get<{ Querystring: { trainerId?: string } }>('/api/trainer/clients', async (request, reply) => {
  const trainerId = Number(request.query.trainerId); if (!Number.isSafeInteger(trainerId) || !trainerAuthorized(request, trainerId)) return reply.code(403).send({ ok: false, error: 'Trainer access required' });
...
app.post<{ Body: { trainerId?: number; clientId?: number } }>('/api/trainer/clients/assign', async (request, reply) => {
  ...
  if (!Number.isSafeInteger(trainerId) || !Number.isSafeInteger(clientId) || !trainerAuthorized(request, trainerId)) return reply.code(403).send({ ok: false, error: 'Trainer access required' });
  const assignment = assignClient(trainerId, clientId); if (!assignment) return reply.code(400).send({ ok: false, error: 'Client assignment failed' }); return { ok: true, assignment };
});
app.get<{ Params: { trainerId: string; clientId: string } }>('/api/trainer/:trainerId/clients/:clientId/progress', async (request, reply) => {
  ...
  if (!Number.isSafeInteger(trainerId) || !Number.isSafeInteger(clientId) || !trainerAuthorized(request, trainerId)) return reply.code(403).send({ ok: false, error: 'Trainer access required' });
  if (!getTrainerClients(trainerId).some((client) => client.id === clientId)) return reply.code(403).send({ ok: false, error: 'Client is not assigned to this trainer' });
  return { ok: true, client: getUser(clientId), history: getWorkoutHistory(clientId) ?? [] };
});
app.get<{ Params: { trainerId: string; clientId: string } }>('/api/trainer/:trainerId/clients/:clientId/schedule', async (request, reply) => {
  const trainerId = Number(request.params.trainerId); const clientId = Number(request.params.clientId);
  if (!Number.isSafeInteger(trainerId) || !Number.isSafeInteger(clientId) || !trainerAuthorized(request, trainerId)) return reply.code(403).send({ ok: false, error: 'Trainer access required' });
  if (!getTrainerClients(trainerId).some((client) => client.id === clientId)) return reply.code(403).send({ ok: false, error: 'Client is not assigned to this trainer' });
  return { ok: true, schedule: getClientSchedule(clientId) };
});
app.put<{ Params: { trainerId: string; clientId: string }; Body: { schedule?: Array<{ day: number; workoutId: string | null }> } }>('/api/trainer/:trainerId/clients/:clientId/schedule', async (request, reply) => {
  ...
  if (!Number.isSafeInteger(trainerId) || !Number.isSafeInteger(clientId) || !trainerAuthorized(request, trainerId)) return reply.code(403).send({ ok: false, error: 'Trainer access required' });
  try {
    const schedule = setClientSchedule(trainerId, clientId, request.body?.schedule ?? [], workouts);
...
app.get<{ Params: { clientId: string } }>('/api/users/:clientId/trainer', async (request, reply) => {
  const clientId = Number(request.params.clientId); const authUser = authenticatedUser(headerInitData(request));
