...
const dataFile=join(dataDir,'new-bot-clients.json');

async function loadProfiles():Promise<Record<string,SavedProfile>>{
  try{return JSON.parse(await readFile(dataFile,'utf8')) as Record<string,SavedProfile>;}
...
async function saveProfile(id:number,user:User,s:Session){
  ...
  profiles[String(id)]={id:profileId,user,profile:{...s},createdAt:profiles[String(id)]?.createdAt??now,updatedAt:now};
