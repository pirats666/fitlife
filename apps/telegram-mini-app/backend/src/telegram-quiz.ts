import { createQuizResult, event, markProgramRequested, markTrainerClicked } from './quiz-store.js';
...
type Update = { message?: { chat: { id: number }; from?: { id: number; username?: string; first_name?: string }; text?: string }; callback_query?: { id: string; from: { id: number }; message?: { chat: { id: number }; message_id: number }; data?: string } };
...
async function send(chatId: number, text: string, replyMarkup?: InlineKeyboard) {
...
function parseStart(text: string) {
  ...
  return { source: params.get('source') ?? undefined, campaign: params.get('campaign') ?? undefined };
}

function resultText(goal: QuizGoal, location: QuizLocation, experience: QuizExperience, program: string) {
  const title = program === 'three_day_split' ? '3-DAY SPLIT — 3 тренировки в неделю' : program === 'full_body_home' ? 'FULL BODY HOME' : program === 'outdoor_full_body' ? 'OUTDOOR FULL BODY' : 'FULL BODY — старт для новичка';
  return `🎯 ТВОЯ СТАРТОВАЯ ТОЧКА\n\nЦель: ${goalLabels[goal]}\nМесто: ${locationLabels[location]}\nУровень: ${experienceLabels[experience]}\n\nТебе сейчас подойдёт:\n${title}\n\n${goalRecommendation(goal)}\n\nНе нужно начинать с огромного количества упражнений или тренироваться каждый день. Важнее выстроить регулярность и постепенно прогрессировать.\n\nЯ подготовил для тебя готовую программу.`;
}
...
export async function handleTelegramUpdate(update: Update) {
  ...
  if (cb.data === 'quiz:start' || cb.data === 'quiz:restart') {
    ...
    clearQuizSession(id); startQuiz(id, next.source, next.campaign);
  ...
  if (cb.data.startsWith('quiz:goal:')) {
    ...
    updateQuizSession(id, { goal }); await event(id, 'QUESTION_1', session.resultId, session.source, session.campaign);
    await send(chatId, 'Отлично 👍\n\nВопрос 2 из 3\n\nГде ты планируешь тренироваться?', locationKeyboard); return;
  }
  if (cb.data.startsWith('quiz:location:')) {
    ...
    updateQuizSession(id, { location }); await event(id, 'QUESTION_2', session.resultId, session.source, session.campaign);
    await send(chatId, 'Понял 👍\n\nВопрос 3 из 3\n\nКакой у тебя сейчас опыт тренировок?', experienceKeyboard); return;
  }
  if (cb.data.startsWith('quiz:experience:')) {
    ...
    await send(chatId, resultText(session.goal, session.location, experience, program), programKeyboard); return;
  }
  if (cb.data === 'quiz:program') {
    if (!session.resultId || !session.program) return;
    await markProgramRequested(session.resultId); await event(id, 'PROGRAM_REQUESTED', session.resultId, session.source, session.campaign);
    await send(chatId, 'Готово 💪\n\nТвоя стартовая программа будет доступна здесь. Сейчас подключим PDF-файлы программ.', afterProgramKeyboard); return;
  }
